...
compute_contribs <- function(pce, categories = CHECK_ITEMS) {
  ...
  anchors <- p %>%
    group_by(LineDescription) %>%
    summarize(
      # Dec '24 → Dec '25 (12-month span): raw Dec/Dec rate
      yoy_2025 = fixed_pair_contrib(
        pick(everything()),
        as.Date("2024-12-01"),
        as.Date("2025-12-01"),
        annualize = FALSE
