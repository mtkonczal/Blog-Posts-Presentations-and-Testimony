...
total_by_year %>%
  ...
  scale_y_continuous(
    breaks = c(
      seq(floor(min(total_by_year$avg_no_children)), 1.7, by = 0.5),
      c(1.8, 1.9, 2.0, 2.1),
      seq(2.2, ceiling(max(total_by_year$avg_no_children)), by = 0.4)
    ),
    minor_breaks = NULL
  ) +
  labs(
    ...
    x = "Year",
    y = "Average number of children",
    caption = "Updated for better y-axis labels. BLS CPS Fertility Supplement via IPUMS; Mike Konczal"
  ) +
  theme_classic(base_size = 13) +
  theme(
    axis.line = element_line(linewidth = 0.25),
