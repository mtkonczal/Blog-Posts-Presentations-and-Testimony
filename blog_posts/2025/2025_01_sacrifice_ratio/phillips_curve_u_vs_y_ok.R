...
pc <- as_tibble(getFRED(c("UNEMPLOY","CLF16OV"),
...
pc <- as_tibble(getFRED(c("gdpc1"),
                        rename_variables = c("real_gdp"),
                        keep_all = FALSE)) %>%
...
pc %>%
  ...
  geom_smooth(method = "lm") +
  geom_path(data = pc %>%
                  filter(year(date) >= 2022), aes(change_u, real_gdp_c), color="darkred", size = 2) +
  theme_classic(base_size = 16) +
  geom_label_repel(data = pc %>% filter(date == max(date) | date == "2022-03-01" | date == max(date) %m-% months(15)), aes(change_u, real_gdp_c, label=format(date, "%b\n%Y")),
                   nudge_y = 0.04) +
  labs(title = "PRELIMINARY: Traveling The Wrong Way Along Okun's 'Law'?",
...
ggsave("g4.png", width = 12, height=6.75, dpi="retina")
