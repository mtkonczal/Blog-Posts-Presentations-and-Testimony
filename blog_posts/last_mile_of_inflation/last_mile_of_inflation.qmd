...
cpi_analysis <- cpi %>% left_join(unrate, by = "date") %>%
...
index <- cpi %>%
  ...
  filter(item_name %in% cpi_lm$item_name, year(date) > 2014) %>%
...
pc_data <- pc_data %>% arrange(date) %>%
  ...
  mutate(core_cpi = (core_cpi/lag(core_cpi,3))^4-1,
...
ggsave("graphics/jolts_PC_jo.png", width = 12, height=12, dpi="retina")
...
ggsave("graphics/jolts_PC_pre_quits.png", width = 12, height=12, dpi="retina")
```

```{r equations}
a <- lm(core_pce ~ quits, data=pc_data[pc_data$date<"2020-01-01",])

b <- lm(core_pce ~ I(quits^2), data=pc_data[pc_data$date<"2020-01-01",])

c <- lm(core_pce ~ quits + I(quits^2), data=pc_data[pc_data$date<"2020-01-01",])

d <- lm(core_pce ~ quits, data=pc_data)

e <- lm(core_pce ~ I(quits^2), data=pc_data)

f <- lm(core_pce ~ quits + I(quits^2), data=pc_data)
...
stargazer(a,b,c,d,e,f,
          type = "latex",
          column.labels = c("1992-2019","1971-2019"),
          dep.var.labels = c("$\\pi_{PCE}$"),
          header=FALSE,
